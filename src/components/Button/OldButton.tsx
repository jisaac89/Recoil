...
export default class Button extends React.Component<IButtonProps, IButtonState> {
	...
	render() {
		...
		let iconPartial =
			props.icon && !props.loading ? props.materialIcon ? (
		...
		let animatedIcon =
			props.iconPointer && !props.loading ? (
				<i className={'icon-pointer fa fa-caret-' + props.iconPointer} />
			) : null;
		...
		let iconWrapperLeft =
			props.icon && props.iconLocation === 'left' ? (
				<div
					className={'icon-pointer-' + props.iconPointer + ' icon-wrapper ' + (props.children ? 'mr10' : '')}
				>
					{iconPartial}
					{props.iconPointer ? animatedIcon : null}
				</div>
		...
		let SimpleButton = ({ innerRef }) => {
			return (
				...
				>
					...
					{props.children}
					{iconWrapperRight}
					<Selectable kind={props.checkedTheme} checked={props.checked ? true : false} />
				</button>
			);
		};
